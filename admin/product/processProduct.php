...
function addProduct()
{
	...
	$sql   = "INSERT INTO tbl_product (cat_id, pd_name, pd_description, pd_price, pd_qty, pd_image, pd_thumbnail, pd_date, pd_last_update)
	          VALUES ('$catId', '$name', '$description', $price, $qty, '$mainImage', '$thumbnail', NOW(), NOW())";
...
function modifythumbnails(){
    $sql = "SELECT pd_image, pd_thumbnail FROM tbl_product where (pd_image!='')";
    $result = dbQuery($sql) or die('Cannot get Product. ' . mysql_error());
    while($row = dbFetchArray($result)) {
        list($bigimage, $thumbimage) = $row;
        if($thumbimage!=''){
            rename( SRV_ROOT . 'images/product/'.$thumbimage,  SRV_ROOT . 'images/product/remainings/'.$thumbimage);
        }
        createThumbnail(SRV_ROOT . 'images/product/'.$bigimage, SRV_ROOT . 'images/product/'.$thumbimage, 100);
